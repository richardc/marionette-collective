...
module MCollective
  describe Shell do
    ...
    describe "#runcommand" do
      %w{ systemu native }.each do |provider|
        describe provider do
          let(:nl) do
          ...
          if provider == 'systemu'
            it "should run the command with systemu" do
              ...
              s = Shell.new("date", :provider => provider)
              s.runcommand
            end
          end

          it "should set stdin, stdout and status" do
            s = Shell.new('ruby -e "STDERR.puts \"stderr\"; STDOUT.puts \"stdout\""', :provider => provider)
            s.runcommand
          ...
          it "should report correct exitcode" do
            s = Shell.new('ruby -e "exit 1"', :provider => provider)
            s.runcommand
          ...
          it "should have correct environment" do
            s = Shell.new('ruby -e "puts ENV[\'LC_ALL\'];puts ENV[\'foo\'];"', :environment => {"foo" => "bar"}, :provider => provider)
            s.runcommand
          ...
          it "should save stdout in custom stdout variable" do
            ...
            s = Shell.new('echo foo', :stdout => out, :provider => provider)
            s.runcommand
          ...
          it "should save stderr in custom stderr variable" do
            ...
            s = Shell.new('ruby -e "STDERR.puts \"foo\""', :stderr => out, :provider => provider)
            s.runcommand
          ...
          it "should run in the correct cwd" do
            ...
            s = Shell.new('ruby -e "puts Dir.pwd"', :cwd => tmpdir, :provider => provider)
          ...
          it "should send the stdin" do
            s = Shell.new('ruby -e "puts STDIN.gets"', :stdin => "hello world", :provider => provider)
            s.runcommand
          ...
          it "should quietly catch Errno::ESRCH if the systemu process has completed" do
            s = Shell.new("echo foo", :provider => provider)
          ...
          describe "timeout has been set" do
            ...
            it "should terminate the systemu process after the specified timeout is exceeded" do
              s = Shell.new(%{ruby -e 'sleep 5'}, :timeout => 1, :provider => provider)
            ...
            it "should kill the systemu process if the parent thread exits and :on_thread_exit is specified" do
              s = Shell.new(%{ruby -e 'sleep 5'}, :timeout => :on_thread_exit, :provider => provider)
              s.stubs(:systemu).yields(1234).returns(@systemu)
              s.stubs(:sleep).with(2)
              Process.stubs(:kill).with(0, 1234).returns(1)
              Process.expects(:kill).with("KILL", 1234)
          ...
          it "should log a warning if the child process cannot be reaped" do
            s = Shell.new('ruby -e "sleep 2"', :timeout => 1, :provider => provider)
          ...
          it "should kill the guard thread when the process returns" do
            s = Shell.new("echo hello world", :provider => provider)
            Thread.stubs(:current)
